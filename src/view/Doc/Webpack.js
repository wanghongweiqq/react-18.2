...
function Webpack () {
  return (
    <div className='pg-doc'>
      ...
      <p>该方法不会添加项目自身的路径，除非你在前面通过__dirname指定了该路径。__dirname是指你操作文件夹的绝对路径，不含该文件名，如本项目操作文件为/config/webpack.config.js，其__dirname为：/Users/wanghongwei/study/react/react-18.2/config</p>
      ...
      <p>其处理方式类似于对这些路径逐一进行cd操作，与cd操作不同的是，这些路径可以是文件，并且可不必实际存在（resolve()方法不会利用底层的文件系统判断路径是否存在，而只是进行路径字符串操作）‌</p>
